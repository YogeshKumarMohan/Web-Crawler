...
AR1 = Audience_review
for item in AR1:
    item['Moviename']= str(item['Moviename'])
pd3= pandas.DataFrame(AR1)
pd6 = pd3['Moviename'].value_counts()
pd6.columns = ['review_counts']
pd6.to_csv("Audience_review_counts_%s.csv"%(year))
...
for item in CR1:
    item['moviename']= str(item['moviename'])
pd4= pandas.DataFrame(CR1)
pd5 = pd4['moviename'].value_counts()


    
pd4.to_csv("Critic_review1_%s.csv"%(year))
pd5.to_csv("Critic_review_counts_%s.csv"%(year))
